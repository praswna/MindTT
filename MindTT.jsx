    ...
    const pathEl = haspath ? (() => {
      ...
      return (
        <svg viewBox={`0 0 ${W} ${H}`} style={{ position:'absolute',inset:0,width:'100%',height:'100%',pointerEvents:'none',zIndex:5 }} preserveAspectRatio="none">
          <defs><marker id="ah" markerWidth="8" markerHeight="6" refX="6" refY="3" orient="auto"><polygon points="0 0,8 3,0 6" fill={sc} opacity="0.9" /></marker></defs>
          <line x1={fx} y1={fy} x2={tx} y2={ty} stroke={sc} strokeWidth="2.5" strokeDasharray="6 3" markerEnd="url(#ah)" opacity="0.85" />
          <text x={(fx+tx)/2} y={(fy+ty)/2-6} fill={sc} fontSize="10" fontWeight="bold" textAnchor="middle" style={{ filter:'drop-shadow(0 1px 2px rgba(0,0,0,0.9))' }}>
